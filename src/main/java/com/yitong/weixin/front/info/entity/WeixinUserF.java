...
public class WeixinUserF extends DataEntity<WeixinUserF> {
	...
	public WeixinUserF(String id){
		super(id);
	}
	
	public WeixinUserF(UserDetailInfo userInfo){
		this.custId = userInfo.getCustId();
		this.openId = userInfo.getOpenid();
		this.userName = userInfo.getNickname();
		this.lastActiveTime = String.valueOf(System.currentTimeMillis() / 1000);
		...
		this.group = new WeixinGroupF("1");
		this.cancelSubscribeTime = "";
	}
	
	public void updateWeixinUser(UserDetailInfo userInfo){
		openId = userInfo.getOpenid();
		userName = userInfo.getNickname();
		acctOpenId = userInfo.getOpenid();
		group = new WeixinGroupF(userInfo.getGroupid());
		sex = userInfo.getSex();
...
//		this.custId = userInfo.getCustId();
	...
	public WeixinGroupF getGroup() {
		return group;
	}
	public void setGroup(WeixinGroupF group) {
		this.group = group;
	}
