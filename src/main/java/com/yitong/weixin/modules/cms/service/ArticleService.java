...
import com.yitong.weixin.modules.cms.dao.ArticleDao;
import com.yitong.weixin.modules.cms.dao.ArticleDataDao;
import com.yitong.weixin.modules.cms.dao.CategoryDao;
...
//		return dao.find(page, dc);
	...
	@Transactional(readOnly = false)
	public Page<Article> findPageByMsgType(Page<Article> page, Article article, boolean isDataScopeFilter) {
		if (article.getCategory()!=null && StringUtils.isNotBlank(article.getCategory().getId()) && !Category.isRoot(article.getCategory().getId())){
			Category category = categoryDao.get(article.getCategory().getId());
			if (category==null){
				category = new Category();
