...
function message(msg, redirect, type) {
	...
	function _message(msg, redirect, type) {
		var modalobj = $('#error-message');
		if(modalobj.length == 0) {
			$(document.body).append('<div id="error-message" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true"></div>');
			var modalobj = $('#error-message');
		}
...
function setReadonlyByArray(array,flag){
	for(var i=0;i<array.length;i++){
		if($("#"+array[i])){
			$("#"+array[i]).attr("readonly",flag);
		}
	}
}