...
<head>
	...
	<script type="text/javascript">
		...
		function view(href){
			top.$.jBox.open('iframe:'+href,'查看模板',$(top.document).width()-220,$(top.document).height()-120,{
				buttons:{"关闭":true},
				loaded:function(h){
...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th style="text-align:center;">选择</th><th>栏目</th><th>标题</th><th>发布者</th><th>更新时间</th></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="article">
			<tr>
				<td style="text-align:center;"><input type="radio" name="id" value="${article.id}" title="${fns:abbr(article.title,40)}" /></td>
